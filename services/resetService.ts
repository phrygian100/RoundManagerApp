  ...
  try {
    ...
    for (const jobDoc of jobsToReset) {
      try {
        ...
        console.error('resetDayToRoundOrder: failed to update', jobId, ':', jobError?.message);
    ...
    if (errors.length > 0) {
      ...
      if (successCount > 0) {
        console.log('resetDayToRoundOrder: returning partial success with', successCount, 'jobs updated');
        return { success: true, jobsReset: successCount };
      } else {
        console.error('resetDayToRoundOrder: all updates failed');
        return { success: false, jobsReset: 0, error: `All updates failed: ${errors.join('; ')}` };
      }
    }

    console.log('resetDayToRoundOrder: all updates succeeded, returning success');
    return { success: true, jobsReset: successCount };
  } catch (error) {
