...
1. User completes job → Firestore job document updates
2. All connected clients receive real-time update via `onSnapshot`
3. UI updates immediately with new job status and completion sequence
4. If out-of-order → swap proposal saved to Firestore
5. All users see the swap proposal in day completion modal

### User-Facing Behavior:
- **Independent vehicle tracking**: Each vehicle's completion sequence is independent (Vehicle A: →1, →2, →3...; Vehicle B: →1, →2, →3...)
- **Shared swap proposals**: Out-of-order suggestions from any user are visible to all users
- **Live updates**: Job completions appear instantly across all devices/sessions
- **Collaborative day completion**: Owner accounts now see swap proposals generated by member users
