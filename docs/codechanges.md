...
- Native: `@react-native-firebase/messaging`, permission + token save after login (`services/pushNotifications.ts`; web stub is a no-op). Notification tap opens runsheet week or `/new-business`. Foreground quote requests also Alert. `app.json` 1.0.2→**1.0.3**, `android.versionCode` 3→**4**, `ios.buildNumber` 3→**4** so Play and side-loaded installs can take a binary that includes the messaging native module.
...
**Regression notes**: Web runsheet uses IndexedDB cache the same way. Cloud Functions `onJobCompleted` and `onQuoteRequestCreated` are deployed. 1.0.2 installs keep receiving OTAs on the 1.0.2 runtime (cache-first). Notifications require installing the 1.0.3 APK/AAB. Upload **1.0.3** to Play, not the earlier 1.0.2 AAB. Owners only get pushes on a device where they have signed in and allowed notifications.
...
- `app/add-client.tsx` - new "✨ Guess Round Order from location" button under "Set Round Order Position". Uses the manually pinned location when set, otherwise geocodes the entered address via the existing `geocodeBestGuess`. On success it opens the normal Set Round Order Position screen pre-scrolled to the guessed slot (via new `initialPosition` param) with a hint line, e.g. "✨ Guessed from location: after 12 Oak Avenue, Springfield (140 m away)" - the user confirms or adjusts there exactly as in the manual flow. Alerts (platform-appropriate) when no address/location is available, geocoding fails, or no existing clients have pins + positions. The client-data payload for the picker is shared between the manual and guessed flows (`buildNewClientData`).
- `app/round-order-position.tsx` - accepts optional `initialPosition` (create mode: starting position, clamped to the valid range, defaults to 1 as before) and `guessHint` (shown under the subtitle) params. All three render paths (desktop web list, mobile browser list, native wheel picker) already centre on the selected position, so the guessed slot is in view on open. The selected row now shows the new client's address (from the add-client form) instead of the "NEW CLIENT" placeholder, in all render paths; the placeholder only appears when no address has been entered yet.

**Regression notes**: confirming in the picker returns the position to the add-client form exactly as the manual flow always has (no DB writes for unsaved clients; consecutive renumbering still happens in the round order manager). Without the new params the picker behaves identically to before, so the edit-existing-client flow and manual create flow are unchanged. The guess itself is a read-only Firestore query.
